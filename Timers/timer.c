...
int main(void)
{
    ...
    set_alternate_pin_function_port1(ALT1,6);       
    P1DIR|=PIN6;
 

    LED_PORT &=  ~(1<<RED_LED);     //Turn off red led
    LED_PORT_DIR |= (1<<RED_LED);   //Set red led Pin as output 


    TA0CCTL1=OUTMOD_7 | CCIE; // Outmod RESET/SET & enable timer interrupts         
    TA0CCR0=1000; 
...
__interrupt void isr_myoverflow(void)
{
   ...
   /* volatile int dummy;
           ...
           LED_PORT &= ~mask(RED_LED);      // Turn OFF red led 
...
{
           // TA0CCR0 = 0;              // Update next match
           TACCTL0 &= ~CCIFG;           // Clear Interrupt flag
           LED_PORT &= mask(RED_LED);   // Turn ON red led
